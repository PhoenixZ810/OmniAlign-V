...
from llava.train.preprocess import preprocess_internlm
from llava.train.dataset import WeightedConcatDataset, ConcatDataset
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    if len(tokenizer.additional_special_tokens_ids) == 2:
        ...
        start_index = tokenizer.additional_special_tokens.index("<|im_start|>")
        end_index = tokenizer.additional_special_tokens.index("<|im_end|>")
        im_start = tokenizer.additional_special_tokens_ids[start_index]
        im_end = tokenizer.additional_special_tokens_ids[end_index]
    nl_tokens = tokenizer("\n").input_ids
...
def load_data(data_path):
    if "jsonl" in data_path:
        data_list = [json.loads(line) for line in open(data_path, "r")]
    else:
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        # data_args.image_folder = ds_collections[ds_name]['root']
        dataset = DPODataset(
            tokenizer=tokenizer,
            data_path=ds_collections[ds_name]['annotation'],
            image_folder=ds_collections[ds_name]['root'],
            data_args=data_args,
...
class DPODataset(Dataset):
    ...
    def __init__(self, data_path: str, image_folder: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
    ...
    def process_image(self, image_file):
        ...
        if self.data_args.image_aspect_ratio == "anyres" or "anyres" in self.data_args.image_aspect_ratio:
...
def make_dpo_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    if data_args.data_path:
        ...
        rank0_print(f'Both data path and meta path are not available!')
        raise
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if "mistral" in model_args.model_name_or_path.lower() or "zephyr" in model_args.model_name_or_path.lower():
    ...
    if model_args.model_class_name is not None:
        ...
        #     )
            ...
            rank0_print("load internlm model")
            ...
            if "zero3" in training_args.deepspeed:
                rank0_print("#### Initialize reference model #####")
                ref_model = LlavaInternlm2ForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation=training_args.attn_implementation,
                    torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
