...
@dataclass
class DataArguments:
    data_path: str = field(default=None, metadata={"help": "Path to the training json data."})
    meta_path: str = field(default=None, metadata={"help": "Path to the meta training jsonl data."})
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        if repeat_time != 1 and local_rank == 0:
            logger.info(
                f'Repeat {data_path} {repeat_time} times, from {len(list_data_dict) // repeat_time} to {len(list_data_dict)}'
            )
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in self.list_data_dict[i]:
            data_dict['image'] = (image, image_size, 'image')
...
def train(attn_implementation=None):
    ...
    if torch.distributed.get_rank() == 0:
        # 创建一个tqdm进度条，显示训练进度
        pbar = tqdm(total=len(train_dataloader), desc="Training Batch", dynamic_ncols=True)
    last_two_batches = []
    for batch in train_dataloader:
        # last_batch = batch  # 最后一个批次的数据
        last_two_batches.append(batch)
        if len(last_two_batches) > 2:
            last_two_batches.pop(0)  # 只保留最近两个批次
            # break
        if torch.distributed.get_rank() == 0:
            pbar.update(1)  # 更新进度条
    rank = torch.distributed.get_rank()
    output_dir = 'debug/llavanext/last_batches'
    os.makedirs(output_dir, exist_ok=True)

    # 构建保存文件的路径，使用rank作为文件名的一部分
    # file_path = os.path.join(output_dir, f'last_2batch_rank_{rank}.json')

    # 将最后一个批次的数据保存为JSON格式
    # with open(file_path, 'w') as f:
    # json.dump(last_batch, f)
    # json.dump([tensor_to_serializable(batch) for batch in last_two_batches], f)

    file_path = os.path.join(output_dir, f'last_2batch_rank_{rank}.pt')
    torch.save(last_two_batches, file_path)
    print(f"Last batch saved to {file_path}")


def tensor_to_list(batch):
    """将batch中的所有tensor转换为列表，以便JSON可以序列化"""
    return {k: (v.tolist() if isinstance(v, torch.Tensor) else v) for k, v in batch.items()}


def tensor_to_serializable(obj):
    """递归转换batch中的Tensor为可序列化格式"""
    if isinstance(obj, torch.Tensor):
        return obj.tolist()  # 将Tensor转为Python列表
